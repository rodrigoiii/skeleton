    ...
    private function pushCoreMiddlewaresDefinition(array &$definitions)
    {
        ...
        if (!empty($middleware_files))
        {
            ...
            foreach ($middlewares as $middleware)
            {
                ...
                if (!array_key_exists("Core\\{$middleware_definition}", $definitions))
                {
                    $definitions["Core\\{$middleware_definition}"] = function(ContainerInterface $c) use ($middleware_definition_value)
                    {
